...
const StudentDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const enrolledRes = await API.get('/students/me/enrollments');
      const enrolledData = enrolledRes.data?.courses || enrolledRes.data || [];
      ...
      try {
        ...
        setRecommendedCourses(recommendedRes.data?.courses || recommendedRes.data || []);
      } catch (recError) {
      ...
      // Fetch achievements
      try {
        const achievementsRes = await API.get('/students/me/achievements');
        setAchievements(achievementsRes.data?.achievements || achievementsRes.data || []);
      } catch (achError) {
        console.warn('Could not fetch achievements:', achError);
        // Default achievements
        setAchievements([
          { id: 1, title: 'First Course Completed', icon: '🎓', unlocked: true, date: new Date().toISOString() },
          { id: 2, title: '3-Day Streak', icon: '🔥', unlocked: true, date: new Date().toISOString() },
          { id: 3, title: 'Perfect Score', icon: '⭐', unlocked: false, date: null },
          { id: 4, title: 'Course Explorer', icon: '🧭', unlocked: false, date: null }
        ]);
      }
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-blue-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {error && (
          <div className={`mb-6 ${error.includes('Successfully') ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} rounded-xl p-4 flex items-start gap-3`}>
            <AlertCircle className={`w-5 h-5 ${error.includes('Successfully') ? 'text-green-600' : 'text-red-600'} mt-0.5 flex-shrink-0`} />
            <div>
              <p className={`${error.includes('Successfully') ? 'text-green-700' : 'text-red-700'} font-medium`}>{error}</p>
            </div>
          </div>
        )}
